/*
 * ------------------------------------------------------------------------------------------
 * This is a modified version of BalusC's FileServlet class.
 * See http://balusc.blogspot.de/2009/02/fileservlet-supporting-resume-and.html
 ...
 * </p>
 *
 * @author BalusC (original author)
 * @author fstab (modifications)
 *
...
public class FileServlet extends HttpServlet {
    ...
    private Long readIdFromRequest(HttpServletRequest request) {
        try {
    ...
    // Request url ends with /static/thumbnail/{id}
    private File readFileFromRequest(HttpServletRequest request) {
        if ( request.getPathInfo() == null ) {
            return null;
        }
        Long id = readIdFromRequest(request);
        if ( id == null ) {
            return null;
        }
        MediaFile file = mediaFileEJB.findById(id);
        if ( file == null ) {
            return null;
        }
        String root = Paths.get(config.findConfig().getMediaRoot(), file.getRootDir()).toString();
        if ( request.getPathInfo().startsWith("/thumbnail") ) {
            return Paths.get(root, file.getGeneratedData().getThumbnail()).toFile();
        }
            ...
            throws IOException
    {
        ...
        // Modification for MyMAM: Call readFileFromRequest() instead
        // of taking the path directly from the request.
        File file = readFileFromRequest(request);
